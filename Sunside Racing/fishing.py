...
Fish go into a bag (saved with missions) and are traded for jungle mastery at the fish
traders in the jungle encampments. Fishing (and fast travel to the beach) opens once any
region reaches FISHING_LEVEL.
...
VALUE = {"common": 1, "uncommon": 2, "rare": 3, "epic": 5}        # Mastery at the trader.
ODDS = {"common": 0.62, "uncommon": 0.27, "rare": 0.10, "epic": 0.01}
...
class FishingSession:
    ...
    def update(self, dt: float, log: FishLog):
        ...
        if self.phase == "cast":
            ...
            self.rarity = roll_rarity(self.rng)
    ...
    def status(self) -> tuple[str, str]:
        ...
        if self.phase == "caught":
            return (f"Caught: {self.rarity.title()} {self.name}!",
                    f"Worth {VALUE[self.rarity]} mastery at a jungle camp trader")
        if self.phase == "escaped":
            return "It got away...", "E to cast again"
        return "", ""

    def sprites(self) -> list[Sprite]:
        """Bobber (or the fish on the line) and the catch held up."""
        out = []
        if self.phase in ("cast", "wait"):
...
def trader_sprites(world) -> list[Sprite]:
    return [Sprite("people-atlas", "fish_trader", x, y, 32, 32, -heading)
...
def beach_destination(world, collisions, car, from_x: float, from_y: float):
    """Fast travel to the beach: the car parks on the sand by the nearest pier."""
    dock = min(world.docks, key=lambda d: math.dist(d.shore(), (from_x, from_y)))
    x, y = dock.shore()
